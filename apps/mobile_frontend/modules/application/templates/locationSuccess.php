...
<?php $p = array(); foreach ($params as $key => $value) { $p[$key] = $value; } ?>
...
<?php echo $location->getRawValue()->renderGetLocationGps(url_for('@application_accept_location?id='.$application->id, true), $p) ?>
<?php else: ?>
<?php echo $location->getRawValue()->renderGetLocationCell(url_for('@application_accept_location?id='.$application->id, true), $p) ?>
<?php endif; ?>
<br>
